...
function findWorkspaceRoot(startDir) {
  ...
  throw new Error(
    'Could not find workspace root (no package.json with workspaces found)',
  );
}
...
function exitWithError(message) {
  ...
  console.warn('Allowing build process to continue (non-fatal for global build)');
  if (metroProcess) {
    metroProcess.kill();
  }
  process.exit(0);
...
function getDeploymentDomain() {
  ...
  console.warn(
    'WARNING: No deployment domain found. Defaulting to localhost. Set REPLIT_INTERNAL_APP_DOMAIN, REPLIT_DEV_DOMAIN, or EXPO_PUBLIC_DOMAIN for production deploys.',
  );
  return 'localhost';
}
...
main().catch((error) => {
  console.warn('Build non-fatal warning:', error.message);
  console.warn('Allowing build process to continue (non-fatal for global build)');
  if (metroProcess) {
    metroProcess.kill();
  }
